...
static int frand_aes_set_key_local(aes_t *aes, const byte *user_key, word32 keylen,
	const byte* iv, int dir)
{
	...
	XMEMCPY(rk, user_key, keylen);
	byte_rev_words(rk, rk, keylen);
        ...
        return frand_aes_set_iv(aes, iv);
}

int frand_aes_set_key(aes_t *aes, const byte *user_key, word32 keylen, const byte *iv,
	int dir)
{
	return frand_aes_set_key_local(aes, user_key, keylen, iv, dir);
}

static void frand_aes_encrypt(aes_t *aes, const byte *in_block, byte *out_block)
{
	word32 s0, s1, s2, s3;
	word32 t0, t1, t2, t3;
	 ...
	 */
	XMEMCPY(&s0, in_block,                  sizeof(s0));
	XMEMCPY(&s1, in_block + sizeof(s0),     sizeof(s1));
	XMEMCPY(&s2, in_block + 2 * sizeof(s0), sizeof(s2));
	XMEMCPY(&s3, in_block + 3 * sizeof(s0), sizeof(s3));

	s0 = byte_rev_word32(s0);
	s1 = byte_rev_word32(s1);
	s2 = byte_rev_word32(s2);
	s3 = byte_rev_word32(s3);
	 ...
	 */
	for (;;) {
		t0 =	te[0][GETBYTE(s0, 3)]  ^
			te[1][GETBYTE(s1, 2)]  ^
			te[2][GETBYTE(s2, 1)]  ^
			te[3][GETBYTE(s3, 0)]  ^
			rk[4];
		t1 =	te[0][GETBYTE(s1, 3)]  ^
			te[1][GETBYTE(s2, 2)]  ^
			te[2][GETBYTE(s3, 1)]  ^
			te[3][GETBYTE(s0, 0)]  ^
			rk[5];
	...
	XMEMCPY(out_block,                  &s0, sizeof(s0));
	XMEMCPY(out_block + sizeof(s0),     &s1, sizeof(s1));
	XMEMCPY(out_block + 2 * sizeof(s0), &s2, sizeof(s2));
	XMEMCPY(out_block + 3 * sizeof(s0), &s3, sizeof(s3));
}
...
int frand_aes_cbc_encrypt(aes_t *aes, byte *out, const byte *in, word32 sz)
{
	...
	while (blocks--) {
		...
		frand_aes_encrypt(aes, (byte *)aes->reg, (byte *)aes->reg);
