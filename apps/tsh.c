 ...
 * 
 * Jeremy King   jerryking21@earthlink.net
 * 
 */
#include <stdio.h>
#include <stdlib.h>
	...
	if (!builtin_cmd(argv)) {
		...
		if (!bg) {
			...
			if(jd!=NULL && jd->state!=ST){
				kill(pid, SIGKILL);
				deletejob(jobs,pid);}    //delete job after it's terminated
		}
		else{
...
void sigtstp_handler(int sig) 
{
	...
	if(fpid!=0){
		...
		kill(fpid, SIGKILL);  //terminate process
		jd->state=ST;}        //change status to stopped
